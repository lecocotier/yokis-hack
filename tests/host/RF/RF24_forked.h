...
public:
 ...
 uint8_t write_register(uint8_t reg,uint8_t v){if(reg==NRF_CONFIG)testRadio.config=v;return 0;}
 uint8_t write_register(uint8_t,const uint8_t*,uint8_t){return 0;}
 ...
 uint8_t write_payload(const void*b,uint8_t n,uint8_t){++testRadio.writes;testRadio.lastPayload.assign((const uint8_t*)b,(const uint8_t*)b+n);testRadio.pending=!testRadio.replies.empty();testRadio.active=true;return 0;}
